...
async function generatePlanilhaVenda(payload = {}) {
  ...
  const requestedMaxClientes = parsePositiveInt(payload.maxClientes, 12);
  ...
  const { outDir, outPath } = resolveOutputTarget(payload.fileName, "Importacao_Venda");

  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet("Venda");
  ...
  const lastClientColumnLetter = worksheet.getColumn(maxColumn).letter;
  ...
  for (let row = 4; row < 4 + maxProdutos; row += 1) {
    ...
    const totalCell = worksheet.getCell(row, 1);
    const productIdCell = worksheet.getCell(row, 2);
    const productNameCell = worksheet.getCell(row, 3);

    totalCell.value = { formula: `SUM(D${row}:${lastClientColumnLetter}${row})` };
...
module.exports = { generatePlanilhaVenda };
